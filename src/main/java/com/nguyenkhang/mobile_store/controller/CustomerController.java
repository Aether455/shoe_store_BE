...
public class CustomerController {
    ...
    public ApiResponse<CustomerResponse> create(@RequestBody @Valid CustomerCreationRequest request){
        return ApiResponse.<CustomerResponse>builder().message("Success!").result(customerService.create(request)).build();
    }

    @GetMapping
    public ApiResponse<Page<CustomerResponse>> getAll(@RequestParam(defaultValue = "0") int page,
                                                      @RequestParam(defaultValue = "10") int size,
                                                      @RequestParam(defaultValue = "id") String sortBy){
        return ApiResponse.<Page<CustomerResponse>>builder().result(customerService.getALl(page, size, sortBy)).build();
    }
    @GetMapping("/{id}")
    public ApiResponse<CustomerResponse> getById(@PathVariable long id){
        return ApiResponse.<CustomerResponse>builder().message("Success!").result(customerService.getById(id)).build();
    }

    @PutMapping("/{id}")
    public ApiResponse<CustomerResponse> update(@PathVariable long id,
                                                @RequestBody @Valid CustomerUpdateRequest request){
        return ApiResponse.<CustomerResponse>builder().message("Success!").result(customerService.update(id, request)).build();
    }
    ...
    public ApiResponse<CustomerResponseForUser> updateByCurrentUser(@RequestBody @Valid CustomerUpdateRequest request){
        return ApiResponse.<CustomerResponseForUser>builder().message("Success!").result(customerService.updateByCurrentUser( request)).build();
    }

    @DeleteMapping("/{id}")
    public ApiResponse<String> delete(@PathVariable long id){
        customerService.delete(id);
        return ApiResponse.<String>builder().message("Success!").result("Size has been deleted").build();
    }
    ...
    public ApiResponse<Page<CustomerResponse>> searchCustomers(
        ...
        return ApiResponse.<Page<CustomerResponse>>builder().result(customerService.searchCustomers(keyword, page)).build();
    }

    @GetMapping("/me")
    public ApiResponse<CustomerResponseForUser> getCustomerByCurrentUser(){
        return ApiResponse.<CustomerResponseForUser>builder().result(customerService.getCustomerByCurrentUser()).build();
    }
}
