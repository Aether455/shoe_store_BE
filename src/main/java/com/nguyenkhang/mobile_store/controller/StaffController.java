...
public class StaffController {
    ...
    public ApiResponse<StaffResponse> create(@RequestBody StaffRequest request) {
        ...
        return ApiResponse.<StaffResponse>builder().message("Success!").result(response).build();
    }

    @GetMapping
    public ApiResponse<Page<StaffResponse>> getStaffs(@RequestParam(defaultValue = "0") int page, @RequestParam(defaultValue = "10") int size, @RequestParam(defaultValue = "id") String sortBy) {
        var response = staffService.getStaffs(page, size, sortBy);
        return ApiResponse.<Page<StaffResponse>>builder().message("Success!").result(response).build();
    }
    ...
    public ApiResponse<String> delete(@PathVariable long staffId) {
        ...
        return ApiResponse.<String>builder().message("Success!").result("Staff has been deleted").build();
    }
    ...
    public ApiResponse<Page<StaffResponse>> searchStaffs(
        ...
        return ApiResponse.<Page<StaffResponse>>builder().result(staffService.searchStaffs(keyword, page)).build();
    }
}
