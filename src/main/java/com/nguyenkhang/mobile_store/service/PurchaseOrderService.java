...
public class PurchaseOrderService {
    ...
    public SimplePurchaseOrderResponse create(PurchaseOrderRequest request){
        var supplier = supplierRepository.findById(request.getSupplierId()).orElseThrow(()->new AppException(ErrorCode.SUPPLIER_NOT_EXISTED));
        var warehouse = warehouseRepository.findById(request.getWarehouseId()).orElseThrow(() -> new AppException(ErrorCode.WAREHOUSE_NOT_EXISTED));
        String name = SecurityContextHolder.getContext().getAuthentication().getName();
        User user = userRepository.findByUsername(name).orElseThrow(() -> new AppException(ErrorCode.USER_NOT_EXISTED));

        Set<Long> variantIds = request.getPurchaseOrderItems().stream().map(PurchaseOrderItemRequest::getProductVariantId).collect(Collectors.toSet());
        ...
        BigDecimal totalAmount = purchaseOrderItems.stream().map(PurchaseOrderItem::getTotal).reduce(BigDecimal.ZERO,BigDecimal::add);
        ...
        log.info("Creating purchase order for supplier {} with {} items",supplier.getName(),purchaseOrderItems.size());
    ...
    public PurchaseOrderResponse getById(long id) {
        var purchaseOrder = purchaseOrderRepository.findById(id).orElseThrow(() -> new AppException(ErrorCode.PURCHASE_ORDER_NOT_EXISTED));
        return purchaseOrderMapper.toPurchaseOrderResponse(purchaseOrder);
    }

    @Transactional
    public PurchaseOrderResponse approve(long id){
        var purchaseOrder = purchaseOrderRepository.findByIdForUpdate(id).orElseThrow(() -> new AppException(ErrorCode.PURCHASE_ORDER_NOT_EXISTED));
        String name = SecurityContextHolder.getContext().getAuthentication().getName();
        User user = userRepository.findByUsername(name).orElseThrow(() -> new AppException(ErrorCode.USER_NOT_EXISTED));


        Map<Long, ProductVariant> variantMap = purchaseOrder.getPurchaseOrderItems().stream().map(PurchaseOrderItem::getProductVariant)
                .collect(Collectors.toMap(ProductVariant::getId, Function.identity()));


        Map<Long,Inventory> inventoriesByVariants = inventoryRepository.findAllByWarehouseIdForUpdate(purchaseOrder.getWarehouse().getId())
                .stream().collect(Collectors.toMap((e)->e.getProductVariant().getId(),Function.identity()));
        ...
        log.info("Approved purchase order {} by user {} with {} items",purchaseOrder.getId(),user.getUsername(),purchaseOrder.getPurchaseOrderItems().size() );
        return purchaseOrderMapper.toPurchaseOrderResponse(purchaseOrder);
    }
    ...
    public PurchaseOrderResponse cancel(long id){
        ...
        log.info("Cancel purchase order {} by user {} with {} items",purchaseOrder.getId(),user.getUsername(),purchaseOrder.getPurchaseOrderItems().size() );
        return purchaseOrderMapper.toPurchaseOrderResponse(purchaseOrder);
    }
    ...
    public Page<SimplePurchaseOrderResponse> searchPurchaseOrder(String keyword, int page){
        ...
        return purchaseOrderRepository.findAll(PurchaseOrderSpecification.createSpecification(keyword), pageable).map(purchaseOrderMapper::toSimplePurchaseOrderResponse);
    }
}
