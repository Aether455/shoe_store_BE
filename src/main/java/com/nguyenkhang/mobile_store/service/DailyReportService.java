...
public class DailyReportService {
    ...
    public void autoCreateDailyReport() {
        ...
        BigDecimal totalRevenue = dailyOrders.stream().map((order) -> BigDecimal.valueOf(order.getFinalAmount())).reduce(BigDecimal.ZERO, BigDecimal::add);

        int totalOrders = dailyOrders.size();

        BigDecimal totalDiscountAmount = dailyOrders.stream().map((e) -> BigDecimal.valueOf(e.getReducedAmount())).reduce(BigDecimal.ZERO, BigDecimal::add);
        ...
        int totalItemsSold = orderItemList.stream().mapToInt(OrderItem::getQuantity).sum();
